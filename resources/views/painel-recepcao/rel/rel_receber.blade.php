...
$tabela = $tabela = contas_receber::where('dat', '>=', $dataInicial)->where('dat', '<=', $dataFinal)->where('pago','LIKE',$status)->get();
...
if($status == 'Sim'){
    $titulo_rel = "RELATÓRIO DE CONTAS A RECEBER PAGAS";
}elseif($status == 'Não'){
    $titulo_rel = "RELATÓRIO DE CONTAS A RECEBER PENDENTES";
}else{
    $titulo_rel = "RELATÓRIO DE CONTAS A PAGAR";
}
...
<table class='table' width='100%' border='1' cellspacing='0' cellpadding='3'>
			<tr bgcolor='#f9f9f9' >
				...
				<td style='font-size:15px'> <b> Pago</b> </td>

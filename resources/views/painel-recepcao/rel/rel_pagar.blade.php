...
$tabela = $tabela = conta_pagar::where('dat', '>=', $dataInicial)->where('dat', '<=', $dataFinal)->where('pago','LIKE',$status)->get();
...
if($status == 'Sim'){
    $titulo_rel = "RELATÓRIO DE CONTAS A PAGAR PAGAS";
}elseif($status == 'Não'){
    $titulo_rel = "RELATÓRIO DE CONTAS A PAGAR PENDENTES";
}else{
